...
function layer4HashBalancedConnectionListener(c) {
  ...
  // Pass connection to worker
  worker = internals.workers[ 
    internals.random.integer(0, internals.workers.length - 1) 
  ];
...
function checkVersion() {
  ...
  if(internals.version.major > 0) {
    ...
    throw new Error(
      'sticky-sessions using layer4 is to old to be used with node ' +
      'major version ' + internals.version.major + 
      ' please check for an udpate!'
    );
  }
  else
  {
    if(internals.version.sub < 9.6) {
      ...
      throw new Error(
        'sticky-sessions using layer4 does not support node version ' + 
        'smaller than 0.9.6 please update node!'
      );
    }
    else if(internals.version.sub < 11.14) {
      ...
      throw new Error(
        'sticky-sessions using layer4 does not support currently node ' +
        'versions greater than 0.11.13, please check for an update!'
      );
    }
  }
}
